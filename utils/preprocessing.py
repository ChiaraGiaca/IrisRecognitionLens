...
ORIGINAL_DATA_DIR = "../data_iiitd/tmp/original_renamed"
CIRCLED_DATA_DIR = "../data_iiitd/tmp/circled"
NORMALIZED_DATA_DIR = "../data_iiitd/tmp/normalized_all"
SYSTEM_DATABASE_DIR = "../data_iiitd/tmp/system_database"

NORMALIZED_IMAGE_WIDTH = 400
NORMALIZED_IMAGE_HEIGHT = 224

def normalize_iris(image: im.Image,
                   output_height: int = NORMALIZED_IMAGE_HEIGHT,
                   output_width: int = NORMALIZED_IMAGE_WIDTH) -> im.Image:
    """
    ...
    theta = np.linspace(0, 2 * np.pi, angle_divisions + 1) #array witch angles for each width pixel
    ...
    sgn = -1 if ox <= 0 else 1 #sign to understand if ther pupil is displeaced on the left or the right of the center

    if ox == 0 and oy > 0: 
        sgn = 1

    a = np.ones(angle_divisions + 1) * (ox ** 2 + oy ** 2) #squared distance

    #calulate th angle for the ceneter of the iris to the point
    if ox == 0:
        phi = np.pi / 2 #90^ 
    else:
        phi = np.arctan(oy / ox) #tangente è oy/ox infatti è sen*r/cos*r

    b = sgn * np.cos(np.pi - phi - theta) #array di coseni --> per normalizzare le distanze

    # Calculate radius around the iris as a function of the angle
    # for each angles theta it calculates the distance between iris and pupil borders
    r = np.sqrt(a) * b + np.sqrt(a * b ** 2 - (a - image.iris.radius ** 2))
    r = np.array([r - image.pupil.radius])

    r_mat = np.dot(np.ones([radius_pixels, 1]), r) #to stretch the iris along the height 

    r_mat = r_mat * np.dot(np.ones([angle_divisions + 1, 1]),
                           np.array([
                               np.linspace(0, 1, radius_pixels) #to normalize
                           ])).transpose()
    ...
    #finds precicely origin coordinates
    xo = r_mat * x_cos_mat 
    yo = r_mat * x_sin_mat
    ...
    polar_array = image.img[yo, xo] #radius
    polar_array = polar_array / 255 #angle
    ...
    return im.Image((polar_array * 255).astype(np.uint8))

#counts how many images failed to process
def circle_available_images(image_paths: List[str], target_dir: str):
    create_empty_dir(target_dir)
    failed_count = 0

    for image_path in image_paths:
        eye_image = im.Image(image_path=image_path)
        user_id, img_type, sample_id = extract_user_sample_ids(image_path)
        try:
            eye_image.circle_iris_and_pupil().save(
                f"{target_dir}/{user_id}_{img_type}_{sample_id}.bmp"
            )
...
#find 100 users, to the fuller one tot the emptier one
def _find_users_with_most_photos(circled_images_paths: List[str],
    ...
    for path in circled_images_paths: #path = image path
...
def split_system_users(circled_images_dir: str = CIRCLED_DATA_DIR,
    ...
    and "unknown", which should be rejected upon identification, this parte is not used though.
    ...
    #take the image paths of the top users
    ...
    for image_path in tqdm(top_users_image_paths,
        ...
        user_id, img_type, _ = extract_user_sample_ids(image_path)
        ...
        if is_registered:
            eye_image = im.Image(image_path=original_image_path)
